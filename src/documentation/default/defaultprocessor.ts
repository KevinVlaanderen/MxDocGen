...
export class DefaultProcessor implements Processor<DefaultTemplateData> {
    ...
    async process(model: IModel): Promise<DefaultTemplateData> {
        return {
            Name: "Documentation",
            Modules: await Promise.all(Array.from(this.modules(model)).map(module => this.processModule(module)))
        }
    }
    ...
    private async processModule(module: IModule) {
        ...
        return {
            ...
            Microflows: {
                ...
                Microflows: await Promise.all(documents.filter(isMicroflow).map(microflow => this.processMicroflow(microflow)))
            }
        }
    }
